...
src/
  ...
  bot/
    ...
    handlers/
      ...
      admin.ts                      # admin:* callback handlers (wizard)<p></p>
...
Everything below is needed before the bot can run in a real Telegram group.

### Critical — bot won't work without these

**1. Environment config**
- Create `.env` from `.env.example`, set a real `BOT_TOKEN`
- Grant the bot admin rights in the group: **Pin Messages** + **Edit Messages**

**2. Missing handler: `admin:members`**
- `adminMenuKeyboard()` shows a "👥 Члени сім'ї" button with action `admin:members`
- No `bot.action('admin:members', ...)` handler exists — pressing it silently fails
- Implement: list current members with name + role + linked status; dad can remove inactive kids

**3. Rule naming — wizard hardcodes "Нове завдання"**
- `initWizard(chatId, 'Нове завдання')` — every rule gets the same display name
- The bot is button-only, so ask the user to pick from preset names or add a `/name_rule` command
- Simplest fix: add a name-picker step at the start of the wizard with common Ukrainian presets (e.g. "Посудомийна машина", "Прибирання", "Сміття") plus a free-text `/set_rule_name <name>` escape hatch

**4. `add_member` only creates kids**
- `registerRegistrationHandlers` adds every member as `'kid'` with auto `kid_order`
- To add mom: dad needs a way to specify role — either a separate `/add_parent` command or a role-picker keyboard after the name is entered (keyboards for it already exist in `registration.ts`: `addMemberRoleKeyboard`)
- Wire up `setup:role:<name>:<role>` action handlers

**5. Startup duty generation**
- The 00:01 cron generates duties, but if the bot is restarted mid-day today's duties are missing until midnight
- On `bot.launch()`, call `generateDutiesForDate(db, family.id, new Date())` for all families (idempotent — safe to call again)

### Quality of life

**6. Unpin when all duties done**
- The spec says: when all duties are done, edit the message to show ✅ and **unpin** it
- `updatePinnedSummary` edits the message but never calls `unpinChatMessage`
- After editing, check `duties.every(d => d.status === 'done')` → call `bot.telegram.unpinChatMessage(chatId, msgId)`

**7. Deployment**
- Add a `Dockerfile` or `pm2.config.js` for process management
- Set `TZ` environment variable to local timezone (e.g. `Europe/Kyiv`) so cron fires at correct wall-clock times

**8. `/list_rules` command for parents**
- No way to see existing rules currently — parents have no visibility into what's configured
- Implement: `/list_rules` replies with a formatted list of all active `work_rules` (name, schedule, rotation mode, workers count)
